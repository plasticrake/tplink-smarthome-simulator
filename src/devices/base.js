...
class Base {
  ...
  get latitude () {
    ...
    } else if (this.data.system.sysinfo.latitude_i !== null) {
  ...
  get longitude () {
    ...
    } else if (this.data.system.sysinfo.longitude_i !== null) {
  ...
  initDefaults () {
    ...
    if (!this.mac) { this.mac = utils.randomMac(); }
    if (!this.deviceId) { this.deviceId = utils.generateId(40); }
    if (!this.hwId) { this.hwId = utils.generateId(32); }
    if (!this.fwId) { this.fwId = utils.generateId(32); }
    if (!this.oemId) { this.oemId = utils.generateId(32); }
    if (!this.latitude) { this.latitude = utils.randomLatitude(); }
    if (!this.longitude) { this.longitude = utils.randomLongitude(); }
  }
}
