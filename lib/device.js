...
let log = logger.getLogger('device');
let logUdp = logger.getLogger('udp');
let logTcp = logger.getLogger('tcp');

Device.prototype.start = function () {
  return new Promise((resolve, reject) => {
    const server = net.createServer((socket) => {
      socket.on('data', (chunk) => {
        logTcp.debug('[%s] TCP DATA', this.model, socket.remoteAddress, socket.remotePort);
        let decryptedMsg = utils.decryptWithHeader(chunk).toString('ascii');
        logTcp.debug(decryptedMsg);
...
Device.prototype.processUdpMessage = function (msg, rinfo) {
  ...
  logUdp.debug('[%s] UDP receiving', this.model, rinfo.port, rinfo.address);
  let msgObj;
  try {
    msgObj = JSON.parse(decyptedMsg);
  } catch (err) {
    logUdp.error('JSON.parse, could not parse:', msgObj);
    throw err;
  }
  logUdp.debug(inspect(msgObj));
  ...
  if (responseObj) {
    ...
    logUdp.debug('[%s] UDP responding', this.model, rinfo.port, rinfo.address);
    logUdp.debug(inspect(responseObj));
...
Device.prototype.processTcpMessage = function (msg, socket) {
  ...
  if (responseObj) {
    ...
    logTcp.debug('[%s] TCP responding', this.model, socket.address());
    logTcp.debug(inspect(responseObj));
